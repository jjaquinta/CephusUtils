...
// Referenced classes of package jo.cephus.core.logic:
//            ParameterizedLogic, ShipDesignLogic
...
public class ShipReportLogic
{
    ...
    public static ShipReportBean report(ShipDesignBean ship)
    {
        try
        {
            ...
            ShipComponentBean jump = reportJump(ship, report, powCode, compRating);
            reportManeuver(ship, report, powCode, jump);
            reportRefinery(ship, report);
            reportSensors(ship, report);
            report.setNumberOfStaterooms(ShipDesignLogic.totalAllInstances(ship, "$stateroom"));
            report.setNumberOfBerths(ShipDesignLogic.totalAllInstances(ship, "$lowberth"));
            report.setNumberOfHardpoints(Math.max(1, report.getHullDisplacement() / 100));
            report.setFireControlTonnage(ShipDesignLogic.totalAllInstances(ship, "BAY"));
            report.setCargoTonnage(ShipDesignLogic.totalAllInstances(ship, "$cargo_hold"));
    ...
    {
        ShipComponentBean sensor = ShipDesignLogic.findHighestTech(ship, "ELECTRONICS");
        if(sensor != null)
        {
            report.setSensorsType(sensor.getName());
            report.setSensorsDM(Integer.parseInt(sensor.getParams().getString("dm")));
        }
    }

    public static void reportRefinery(ShipDesignBean ship, ShipReportBean report)
    {
        double refinery = ShipDesignLogic.volumeAllInstances(ship, "$fuel_processor");
        if(refinery > 0.0D)
        {
            int tonsPerDay = (int)(20D * refinery);
            report.setFuelUsageNotes(AudioMessageBean.group(new Object[] {
                report.getFuelUsageNotes(), new AudioMessageBean("FUEL_REFINERY_CAN_REFINE", new Object[] {
                    Integer.valueOf(tonsPerDay)
                })
            }));
        }
    }
    ...
    {
        ShipComponentBean power = ShipDesignLogic.findHighestStringParam(ship, "PPLANT", "driveCode");
        String powCode;
        if(power == null)
            powCode = " ";
        else
    ...
    {
        ShipComponentBean comp = ShipDesignLogic.findHighestStringParam(ship, "COMPUTER", "rating");
        int compRating = 0;
        if(comp == null)
        {
        ...
        {
            compRating = IntegerUtils.parseInt(comp.getParams().getString("rating"));
            report.setComputerModel(comp.getName());
            List<ShipComponentInstanceBean> bis = ShipDesignLogic.getAllInstances(ship, "$computer_bis");
            if(bis.size() > 0)
            {
                report.setComputerModel(AudioMessageBean.group(new Object[] {
                    report.getComputerModel(), ((ShipComponentInstanceBean)bis.get(0)).getComponent().getName()
                }));
                compRating += 5;
            }
            List<ShipComponentInstanceBean> fib = ShipDesignLogic.getAllInstances(ship, "$computer_fib");
            if(fib.size() > 0)
                report.setComputerModel(AudioMessageBean.group(new Object[] {
                    report.getComputerModel(), ((ShipComponentInstanceBean)fib.get(0)).getComponent().getName()
                }));
        }
        return compRating;
    }

    public static ShipComponentBean reportJump(ShipDesignBean ship, ShipReportBean report, String powCode, int compRating)
    {
        ShipComponentBean jump = ShipDesignLogic.findHighestStringParam(ship, "JDRIVE", "driveCode");
        if(jump == null)
        {
            report.setJumpCode(" ");
        ...
        } else
        {
            ...
            report.setJumpNumber(ShipDesignLogic.getDrivePerformance(jumpCode, report.getHullDisplacement()));
            if(powCode.compareTo(jumpCode) < 0)
                report.getErrors().add((new StringBuilder("Underpowered. Power plant '")).append(powCode).append("' is less than jump drive '").append(jumpCode).append("'").toString());
            ...
            if(compRating / 5 < report.getJumpNumber())
                report.getErrors().add((new StringBuilder("Computer has rating of ")).append(compRating).append(", but it needs ").append(report.getJumpNumber() * 5).append(" to control jump-").append(report.getJumpNumber()).toString());
        }
        return jump;
    }

    public static void reportManeuver(ShipDesignBean ship, ShipReportBean report, String powCode, ShipComponentBean jump)
    {
        ShipComponentBean man = ShipDesignLogic.findHighestStringParam(ship, "MDRIVE", "driveCode");
        if(man == null)
        {
        ...
        } else
        {
            ...
            report.setThrustNumber(ShipDesignLogic.getDrivePerformance(manCode, report.getHullDisplacement()));
            ...
            if(powCode.compareTo(manCode) < 0)
                report.getErrors().add((new StringBuilder("Underpowered. Power plant '")).append(powCode).append("' is less than maneuver drive '").append(manCode).append("'").toString());
            report.setFuelUsageNotes(new AudioMessageBean("POWER_PLANT_USES", new Object[] {
                Integer.valueOf(fuelPerWeek)
            }));
        }
    }
    ...
    {
        List<ShipComponentInstanceBean> hangers = ShipDesignLogic.getAllInstances(ship, "$hanger");
        List<AudioMessageBean> hangersDesc = new ArrayList<>();
        AudioMessageBean hangerDesc;
        for(Iterator<ShipComponentInstanceBean> iterator = hangers.iterator(); iterator.hasNext(); hangersDesc.add(hangerDesc))
        {
            ShipComponentInstanceBean hanger = iterator.next();
            hangerDesc = new AudioMessageBean("TON", new Object[] {
                Integer.valueOf(hanger.getCount()), hanger.getComponent().getName()
            });
        }
    ...
    {
        List<ShipComponentInstanceBean> screens = ShipDesignLogic.getAllInstances(ship, ShipComponentBean.SCREENS);
        List<AudioMessageBean> screenDesc = new ArrayList<>();
        for(Iterator<ShipComponentInstanceBean> iterator = screens.iterator(); iterator.hasNext();)
        {
    ...
    public static void reportTurrets(ShipDesignBean ship, ShipReportBean report)
    {
        ...
        {
            ShipComponentInstanceBean weapon = (ShipComponentInstanceBean)weapons.get(i);
        ...
        for(Iterator<ShipComponentInstanceBean> iterator = turrets.iterator(); iterator.hasNext();)
        {
            ...
            for(int i = 0; i < turret.getCount(); i++)
            {
                ...
                AudioMessageBean turretDesc = AudioMessageBean.group(new Object[] {
                    turret.getComponent().getName(), TextLogic.compress(contents)
                });
    ...
    {
        int tl = 0;
        for(Iterator<ShipComponentInstanceBean> iterator = components.iterator(); iterator.hasNext();)
        {
